...
def save_knots(file_name,knots):
    i=0
    f_name='000_'+'_'.join(file_name)

    while os.path.isfile(f_name):
        f_name='{0:{fill}>3s}_{1}'.format(str(i), '_'.join(file_name), fill='0')
        i+=1
...
def read_knots(f_name):
    try:
        ...
        print('file: {0} --ERROR'.format(f_name))
    else:
        with f:
            data=[]
            for line in f:
...
def knots2face(knots, t, offset):
    ...
    if t!=0:
        n_vect = cross(vect(knots_list[0], knots_list[1]) ,\
                      vect(knots_list[-1], knots_list[-2]),'normalize')
        face = face.extrude(Base.Vector(n_vect[0]*-t , n_vect[1]*-t , n_vect[2]*-t))
...
parser.add_argument('-i1','--master_input_knots',type=str,required=True,help='master input vertex list/s')
parser.add_argument('-i2','--slave_input_knots',type=str,help='slave input vertex list')

parser.add_argument('-o','--output_knots',type=str,help='output file name')
parser.add_argument('-fc','--fc_model_output_name',type=str,help='generate FreeCAD model')
parser.add_argument('-fct','--fc_model_thickness',default=dflt_fc_model_thickness,type=float,help='FreeCAD model sheet thickness')
...
prof_f = args.master_input_knots
fold_f = args.slave_input_knots
...
if prof_knots:
    ...
    unfold_faces_list=[[interp_series(X,x_len,p1[0])-x0_len,p1[1],0] for p1 in pool_list]
    save_knots(output_f , unfold_faces_list)
    ...
    if fc_model_name:
        ...
        for b1, b2 in zip(folds_X,folds_X[1::]):
            fc_faces_list.append([var for var in pool_list if b1 <= var[0] <= b2])

        for face in fc_faces_list:
            fc_fold_faces_list.append([[p1[0],p1[1],interp_series(X,Y,p1[0])] for p1 in face])
        ...
        myPart.Shape=Part.Compound([knots2face(face,sheet_thickness,0) for face in fc_fold_faces_list])
